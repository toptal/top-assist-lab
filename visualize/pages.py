...
def import_data(session):
    ...
    page_ids, all_documents, embeddings_json = PageManager().get_all_page_data_from_db(session)
...
def load_confluence_pages_spacial_distribution(session):
    print("Starting 3D visualization process...")

    page_ids, all_documents, embeddings_json = import_data(session)
